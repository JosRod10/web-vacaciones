...
export class PanelControlVac {
  ...
  descargarExcel(){
      ...
      this.solicitudes.map((ele: any)=> {
        const solicitud = {
          ...
          Vacaciones_tomadas: ele.Vacaciones_tomadas,
          Saldo: ele.Saldo
        }
  ...
  formatearFecha(fechaISO: string): string {
    ...
    // 'es-GB' utiliza el formato DD/MM/YYYY
    return fecha.toLocaleDateString('es-GB', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric'
    });
  ...
  consultarControl(){
    this.api.consultarControl().subscribe(
      ...
      (err)=>{
          console.log('Error al cargar control: ',err);
      }
    )
  }
